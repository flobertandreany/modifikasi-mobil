<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateModificationDetailTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('modification_details', function (Blueprint $table) {
            $table->id();
            $table->foreignId('modification_id')->constrained('modifications');
            $table->foreignId('car_brand_id')->constrained('car_brands');
            $table->foreignId('car_model_id')->constrained('car_models');
            $table->string('car_year');
            $table->foreignId('car_engine_id')->constrained('car_engines');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('modification_details');
    }
}
